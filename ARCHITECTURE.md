...
- **状态展示层**：`src/ui/*`
  ...
  - `src/ui/interaction/panel.rs`：互动面板渲染（`InteractionPanel`，浮动 Window + 左侧分类选项卡）。
- **状态计算层**：`src/stats/*`
...
6. 绑定交互：输入探针、长按拖拽、头/身体点击区域、右键菜单（投喂分类子菜单 + 互动子菜单 + 系统子菜单）。
7. 创建并连接 `StatsPanel`，并启动配置文件热更新监听。
8. 创建并连接 `FeedPanel` 与 `InteractionPanel`（互动菜单点击后打开互动面板并切换到对应分类）。
...
- 状态面板、投喂面板与互动面板默认手动关闭（点击面板内“退出”按钮或窗口关闭按钮）。
- 右键再次弹出主菜单前会统一执行面板隐藏回调，避免多面板叠加遮挡。

### 6.1.3 互动分类面板（新增）

- 互动入口在右键菜单 `互动` 子菜单，包含：`学习/工作/玩耍`。
- 互动菜单点击行为：
  1. 打开 `InteractionPanel` 浮动窗口；
